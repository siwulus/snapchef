...
- [x] 1.3 Sign-in: valid → `/recipes`; bad email → field error; wrong credentials → supabase message inline
- [x] 1.4 Sign-up: per-field validation errors; success → `/auth/confirm-email`
- [x] 1.5 Sign-out from the topbar still works
...
- [ ] 2.3 No references remain: `grep -r "submitJson" src` exits empty

#### Manual

- [ ] 2.4 Sign-in / sign-up flows behave as in Phase 1 — now through `useApiClient`
- [ ] 2.5 Offline submit shows toast; no unhandled rejection in console
- [ ] 2.6 Review: `src/lib/CLAUDE.md` matrix + root `CLAUDE.md` match implemented layering
