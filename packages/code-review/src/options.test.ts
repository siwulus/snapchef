...
describe("CliOptions schema", () => {
  it("applies documented defaults when nothing is supplied", () => {
    ...
    expect(parsed).toEqual({ json: false, model: DEFAULT_MODEL });
    expect(DEFAULT_MODEL).toBe("claude-sonnet-4-6");
  });

  it("honors explicit values", () => {
    const parsed = CliOptions.parse({ json: true, model: "claude-opus-4-8" });
    expect(parsed).toEqual({ json: true, model: "claude-opus-4-8" });
  });
